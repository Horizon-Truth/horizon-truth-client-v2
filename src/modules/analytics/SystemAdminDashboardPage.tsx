...
import {
    Users, Building2, Gamepad2, BookOpen, MessageSquare,
    FileText, Mail, PlayCircle, TrendingUp, AlertCircle, Loader2, Cpu
} from "lucide-react";
...
export default function SystemAdminDashboardPage() {
    ...
    if (isError) {
        return (
            <div className="flex h-[80vh] flex-col items-center justify-center gap-4 text-center">
                <AlertCircle className="h-12 h-12 text-destructive" />
    ...
    const cards = [
        ...
        { title: "Guest Plays", value: o?.guestPlays, icon: PlayCircle, color: "text-pink-500", bg: "bg-pink-50/50" },
        { title: "Scenarios", value: o?.scenarios, icon: Cpu, color: "text-emerald-500", bg: "bg-emerald-50/50" },
        { title: "Feedback", value: o?.feedback, icon: MessageSquare, color: "text-orange-500", bg: "bg-orange-50/50" },
        { title: "Reports", value: o?.reports, icon: AlertCircle, color: "text-red-500", bg: "bg-red-50/50" },
        { title: "Blog Posts", value: o?.blogs, icon: FileText, color: "text-cyan-500", bg: "bg-cyan-50/50" },
        { title: "Resources", value: o?.resources, icon: BookOpen, color: "text-teal-500", bg: "bg-teal-50/50" },
        { title: "Contact Reports", value: o?.contacts, icon: Mail, color: "text-rose-500", bg: "bg-rose-50/50" },
    ];

    const barData = [
        { name: 'Users', count: o?.users },
        { name: 'Players', count: o?.players },
        { name: 'Guests', count: o?.guestPlays },
    ];
    ...
    return (
        <motion.div className="space-y-8 p-1" initial="hidden" animate="visible" variants={container}>
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1 font-medium">
                        Full platform overview — users, content, engine, and system health.
                    </p>
                </div>
                <div className="flex gap-2 flex-wrap">
                    <Button asChild variant="outline" className="rounded-xl font-semibold border-2">
                        <Link to="/dashboard/users">User Directory</Link>
                    </Button>
                    <Button asChild variant="outline" className="rounded-xl font-semibold border-2">
                        <Link to="/dashboard/engine">Scenario Engine</Link>
                    </Button>
                    <Button asChild className="rounded-xl font-bold shadow-lg shadow-primary/20">
                        <Link to="/dashboard/health">
                            <TrendingUp className="mr-2 h-4 w-4" />System Health
                        </Link>
                    </Button>
                </div>
            </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                ...
                <motion.div variants={item} className="rounded-2xl border bg-card p-6 shadow-sm flex flex-col h-[380px]">
                    <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                        <MessageSquare className="h-5 w-5 text-orange-500" />Feedback Summary
                    </h3>
                    <div className="flex-1">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie data={feedbackDist} cx="50%" cy="50%" outerRadius={90} dataKey="value"
                                    label={({ name, percent }) => `${name} ${((percent || 0) * 100).toFixed(0)}%`}>
                                    {feedbackDist.map((_, i) => <Cell key={i} fill={COLORS[(i + 3) % COLORS.length]} />)}
                                </Pie>
                                <Tooltip contentStyle={{ borderRadius: 12, border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} />
                            </PieChart>
                        </ResponsiveContainer>
            ...
            <motion.div variants={item} className="grid gap-6 md:grid-cols-3">
                    ...
                    <div className="flex flex-col md:flex-row gap-8 items-center">
                        <div className="flex-1 space-y-4">
                            ...
                            <p className="opacity-90 leading-relaxed font-medium">
                                {o?.blogs} blog posts and {o?.resources} resources published to date.
                            </p>
                            <div className="flex gap-4 pt-2">
                                <Button asChild className="bg-white text-primary hover:bg-white/90 rounded-xl font-bold">
                                    <Link to="/dashboard/resources/blogs">Manage Blog</Link>
                                </Button>
                                <Button asChild variant="outline" className="border-white/30 hover:bg-white/10 rounded-xl font-bold">
                                    <Link to="/dashboard/resources/library" className="text-white">Manage Resources</Link>
                                </Button>
                            </div>
                        </div>
                        <div className="hidden lg:flex gap-4">
                            <div className="bg-white/20 backdrop-blur-md rounded-2xl p-6 text-center w-28">
                                <div className="text-3xl font-black">{o?.blogs}</div>
                                <div className="text-xs font-bold uppercase mt-1 opacity-80">Blogs</div>
                            </div>
                            <div className="bg-white/20 backdrop-blur-md rounded-2xl p-6 text-center w-28">
                                <div className="text-3xl font-black">{o?.resources}</div>
                ...
                <div className="rounded-2xl border bg-card p-6 shadow-sm border-l-4 border-l-rose-500">
                    ...
                    <div className="p-3 rounded-xl bg-muted/50 border border-border/50">
                        ...
                        <div className="text-2xl font-black mt-1">{o?.contacts}</div>
                    </div>
                    <Button asChild variant="ghost" className="w-full justify-between rounded-xl hover:bg-rose-50 hover:text-rose-600 font-bold mt-4">
                        <Link to="/dashboard/contacts">View Inquiries<TrendingUp className="h-4 w-4" /></Link>
                    </Button>
                </div>
            </motion.div>
        </motion.div>
